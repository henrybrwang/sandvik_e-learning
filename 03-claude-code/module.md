...
Sandvik's Claude Enterprise is direct and consumption-based, with no five-hour session or weekly limits (those apply only to individual Claude plans); usage is pay as you go and charged to the user's cost center. Each new license starts with a default limit of $25 per user per month. If a user needs a higher monthly usage or spending limit for business work, they must use the intranet form and get approval from their cost approver. The monthly spending limit should be visible in the user's Claude account.
...
Complete the quiz after reading this module. The five quiz questions check the demo-versus-dependable mindset, planning before a non-trivial change, naming the task intent, keeping secrets and production data out of a session, and treating AI-written tests as evidence rather than proof.
